...
| Rollback / stale release | `sequence` + `expiresAt` + highest-seen sequence policy | 9 |
...
| Announce DoS | Rate limit, bounded payload, TTL, candidate limit | 3 |
| Cache poisoning | Only verified hash-addressed chunks are seedable | 1 / 7 |
| Secret leakage in logs | Tokens and keys never logged | 9 |
